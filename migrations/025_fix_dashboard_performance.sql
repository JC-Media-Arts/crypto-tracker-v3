...
-- 4. Refresh materialized views (skip if they don't exist)
-- Note: Run these manually if the views exist in your database
-- REFRESH MATERIALIZED VIEW CONCURRENTLY ohlc_recent;
-- REFRESH MATERIALIZED VIEW CONCURRENTLY ohlc_today;

-- 5. Create a strategy status cache table for pre-calculated data
...
-- Note: To increase timeout (requires admin access):
