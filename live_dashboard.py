...
def get_engine_status():
    ...
    try:
        ...
        result = subprocess.run(['pgrep', '-f', 'run_paper_trading'], 
    ...
    try:
        ...
        if result.data:
            ...
            for symbol in symbols:
                try:
                    price_result = db.client.table("ohlc_data")\
                        .select("close")\
                        .eq("symbol", symbol)\
                    ...
                    for i, buy_trade in enumerate(symbol_trades['buys']):
                        if i not in matched_buys and buy_trade['created_at'] <= sell_trade['created_at']:
                            ...
                            trades_data.append({
                                ...
                                'dot_color': dot_color,
                                'sl_display': f"${buy_trade['stop_loss']:.2f}" if buy_trade.get('stop_loss') else None,
                                'tp_display': f"${buy_trade['take_profit']:.2f}" if buy_trade.get('take_profit') else None,
                                'ts_display': f"{float(buy_trade.get('trailing_stop_pct', 0))*100:.1f}%" if buy_trade.get('trailing_stop_pct') else None,
                ...
                for i, buy_trade in enumerate(symbol_trades['buys']):
                    if i not in matched_buys and buy_trade['status'] == 'FILLED':
                        ...
                        if buy_trade.get('strategy_name') == 'DCA':
                            ...
                            all_buys_for_symbol = [t for t in symbol_trades['buys'] 
                                                  if t['status'] == 'FILLED' and 
                                                  t.get('strategy_name') == 'DCA']
                            dca_fills = len(all_buys_for_symbol)
                            
                            # Calculate average entry price if multiple fills
                            if dca_fills > 1:
                                total_cost = sum(float(t['price']) * float(t.get('amount', 1)) for t in all_buys_for_symbol)
