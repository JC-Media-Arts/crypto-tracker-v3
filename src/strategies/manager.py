...
class StrategyManager:
    ...
    def __init__(self, config: Dict, supabase_client=None):
        ...
        logger.info(f"Allocation: DCA {self.allocation.dca_allocation:.0%}, Swing {self.allocation.swing_allocation:.0%}, Channel {getattr(self.allocation, 'channel_allocation', 0.3):.0%}")
    ...
    async def scan_for_opportunities(self, market_data: Dict) -> List[StrategySignal]:
        ...
        logger.info(f"Found {len(signals)} total opportunities: "
            ...
            if not setup:
                ...
                if self.scan_logger:
                    ...
                    self.scan_logger.log_scan_decision(
                        ...
                        market_regime=regime.value,
                        btc_price=btc_price,
                        thresholds_used={'drop_threshold': self.config.get('dca_config', {}).get('drop_threshold', -5.0)}
                    )
                continue
        ...
        for signal in signals:
            if signal.strategy_type == StrategyType.DCA:
                ...
                if signal.required_capital <= available:
                    ...
                    logger.warning(f"Insufficient DCA capital for {signal.symbol}: "
                ...
                if signal.required_capital <= available:
                    ...
                    logger.warning(f"Insufficient Swing capital for {signal.symbol}: "
                                 f"needs ${signal.required_capital:.2f}, have ${available:.2f}")
                ...
                if signal.required_capital <= available:
                    ...
                    logger.warning(f"Insufficient Channel capital for {signal.symbol}: "
                                 f"needs ${signal.required_capital:.2f}, have ${available:.2f}")
        
        return approved_signals
    
    def _calculate_expected_value(self, win_prob: float, expected_profit: float, 
                                 expected_loss: float, confidence: float) -> float:
    ...
    def _calculate_priority_score(self, strategy: StrategyType, 
                                 expected_value: float, confidence: float) -> float:
        """
    ...
    async def _execute_dca_signal(self, signal: StrategySignal) -> bool:
        ...
        try:
            ...
            logger.info(f"Executing DCA grid for {signal.symbol}: "
    ...
    async def _execute_swing_signal(self, signal: StrategySignal) -> bool:
        ...
        try:
            ...
            logger.info(f"Executing Swing trade for {signal.symbol}: "
    ...
    def update_performance(self, symbol: str, pnl: float, is_win: bool):
        ...
        if symbol in self.active_positions:
            ...
            logger.info(f"Updated {strategy.value} performance: "
