...
class ChannelExecutor:
        ...
        for symbol, ohlc_data in market_data.items():
            ...
            if signal:
                ...
                if targets.get('risk_reward', 0) < self.min_risk_reward:
                    logger.debug(f"Skipping {symbol}: R:R {targets['risk_reward']:.2f} "
                ...
                if position:
                    ...
                    logger.info(f"Channel trade opened: {symbol} {signal} at ${current_price:.2f}, "
    ...
    async def _create_position(self, symbol: str, channel: Channel, signal: str,
                              entry_price: float, targets: Dict) -> Optional[ChannelPosition]:
        """
    ...
    async def monitor_positions(self, market_data: Dict[str, List[Dict]]) -> List[Dict]:
        ...
        for symbol, position in list(self.positions.items()):
            ...
            if exit_reason:
                ...
                logger.info(f"Channel position closed: {symbol} at ${current_price:.2f}, "
                           f"P&L: ${position.pnl:.2f}, Reason: {exit_reason}")
        
        return closed_positions
    
    def _check_exit_conditions(self, position: ChannelPosition, 
                              current_price: float, ohlc_data: List[Dict]) -> Optional[str]:
        """
    ...
    async def _close_position(self, position: ChannelPosition, 
