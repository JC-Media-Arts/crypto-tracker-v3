...
class SignalGenerator:
    ...
    async def scan_for_signals(self) -> List[Dict]:
        ...
        try:
            ...
            available_symbols = [
                s for s in symbols 
                if not self._is_symbol_blocked(s)
            ]
    ...
    async def _apply_ml_filtering(self, signal: Dict) -> Dict:
        ...
        try:
            ...
            expected_value = (
                predictions["take_profit_percent"] * confidence -
                abs(predictions["stop_loss_percent"]) * (1 - confidence)
            )
    ...
    async def _prepare_ml_features(self, signal: Dict) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            expected_features = [
                'price_change_24h', 'high_low_ratio', 'volume_ratio',
                'volatility', 'rsi', 'regime_bull', 'regime_bear',
                'regime_neutral', 'drop_magnitude', 'support_distance',
    ...
    async def _calculate_position_size(self, signal: Dict) -> float:
        ...
        try:
            ...
            position_size = self.position_sizer.calculate_position_size(
                ...
                if signal["ml_predictions"] else 1.0
            )
    ...
    def _get_symbols_to_monitor(self) -> List[str]:
        ...
        return [
            "BTC", "ETH", "SOL", "ADA", "DOT", "AVAX", "LINK", 
            "UNI", "ATOM", "NEAR", "MATIC", "ARB", "OP", "INJ",
            "TIA", "SEI", "SUI", "APT", "FTM", "ALGO"
        ]
        ...
        return [
            {
                ...
                "detected_at": s["detected_at"].isoformat() if isinstance(s["detected_at"], datetime) else s["detected_at"],
