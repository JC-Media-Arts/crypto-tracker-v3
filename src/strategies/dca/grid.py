...
class GridCalculator:
    ...
    def validate_grid(self, grid: Dict, account_balance: float) -> Tuple[bool, str]:
        ...
        if grid['total_investment'] > account_balance:
            return False, f"Insufficient balance: need ${grid['total_investment']:.2f}, have ${account_balance:.2f}"
        
        # Check minimum order sizes
        for level in grid['levels']:
            if level['size'] < 10:  # $10 minimum
                return False, f"Order size too small at level {level['level']}: ${level['size']:.2f}"
