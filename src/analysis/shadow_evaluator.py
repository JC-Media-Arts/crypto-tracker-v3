...
class ShadowEvaluator:
    ...
    async def _get_pending_shadows(self) -> List[Dict]:
        ...
        try:
            ...
            result = self.supabase.table('shadow_variations')\
    ...
    async def _evaluate_shadow(self, shadow: Dict) -> Optional[ShadowOutcome]:
        ...
        try:
            # Get scan details
            scan_data = shadow.get('scan_history', {}) if 'scan_history' in shadow else await self._get_scan_data(shadow['scan_id'])
            if not scan_data:
    ...
    async def _evaluate_single_entry_shadow(self,
                                           shadow: Dict,
                                           price_data: List[Dict],
                                           entry_time: datetime) -> ShadowOutcome:
        """
    ...
    async def _evaluate_dca_shadow(self,
                                  shadow: Dict,
                                  price_data: List[Dict],
                                  entry_time: datetime) -> ShadowOutcome:
        """
    ...
    async def _get_price_data(self,
        ...
        try:
            # Query OHLC data
            result = self.supabase.table('ohlc_data')\
    ...
    async def _check_real_trade_match(self, outcome: ShadowOutcome) -> bool:
        ...
        try:
            ...
            shadow_result = self.supabase.table('shadow_variations')\
            ...
            if shadow_result.data['variation_name'] != 'CHAMPION':
                ...
                trade_result = self.supabase.table('trade_logs')\
