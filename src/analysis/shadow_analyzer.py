...
class ShadowAnalyzer:
    ...
    async def _generate_variation_recommendations(
        self, best_performer: PerformanceMetrics, strategy: str
    ...
    def _determine_confidence_level(self, trade_count: int, outperformance: float, p_value: float, days: int) -> str:
        ...
        for tier_name, tier_config in ShadowConfig.CONFIDENCE_TIERS.items():
            if (
                trade_count >= tier_config["min_trades"]
                and outperformance >= tier_config["min_outperformance"]
                and p_value <= tier_config["max_p_value"]
                and days >= tier_config["min_days"]
            ):
                return tier_name
