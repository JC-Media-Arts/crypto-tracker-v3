...
class ShadowConfig:
    ...
    def should_adjust_parameter(cls, param_name: str, evidence: Dict) -> tuple[bool, float, str]:
        ...
        for tier_name, tier_config in cls.CONFIDENCE_TIERS.items():
            if (evidence['trade_count'] >= tier_config['min_trades'] and
                evidence['outperformance'] >= tier_config['min_outperformance'] and
                evidence['p_value'] <= tier_config['max_p_value'] and
                evidence['consistent_days'] >= tier_config['min_days']):
