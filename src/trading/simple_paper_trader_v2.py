...
class SimplePaperTraderV2:
    ...
    def __init__(self, initial_balance: float = 10000.0, max_positions: int = 50):
        ...
        self.mid_cap = [
            "SOL", "BNB", "XRP", "ADA", "AVAX", "DOT", "LINK",
            "ATOM", "UNI", "NEAR", "ICP", "ARB", "OP", "AAVE",
            "CRV", "MKR", "LDO", "SUSHI", "COMP"
        ]
    ...
    async def update_daily_performance(self, trade: Trade):
        ...
        try:
            ...
            existing = self.db_client.client.table("paper_performance").select("*").eq("date", date).eq("strategy_name", trade.strategy).eq("trading_engine", "simple_paper_trader").execute()
            ...
            if existing.data:
                ...
                self.db_client.client.table("paper_performance").update(updated).eq("date", date).eq("strategy_name", trade.strategy).eq("trading_engine", "simple_paper_trader").execute()
            else:
                # Create new record
                data = {
                    "date": date,
