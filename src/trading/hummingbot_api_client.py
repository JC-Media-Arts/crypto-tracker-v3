...
class HummingbotAPIClient:
    ...
    async def get_positions(self, bot_id: str) -> List[HummingbotPosition]:
        ...
        try:
            async with self.session.get(
                f"{self.base_url}/bots/{bot_id}/positions",
                headers=self._get_headers()
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    positions = []
                    for pos in data:
                        positions.append(HummingbotPosition(
                            symbol=pos["symbol"],
                            side=pos["side"],
                            amount=pos["amount"],
    ...
    async def get_balance(self, bot_id: str) -> Dict[str, float]:
        ...
        try:
            async with self.session.get(
                f"{self.base_url}/bots/{bot_id}/balance",
                headers=self._get_headers()
            ) as response:
                if response.status == 200:
                    data = await response.json()
    ...
    async def execute_dca_grid(self,
        ...
        try:
            ...
            logger.info(f"DCA grid executed for {symbol}: {len(grid_orders)} orders, TP: {take_profit}, SL: {stop_loss}")
            return True
