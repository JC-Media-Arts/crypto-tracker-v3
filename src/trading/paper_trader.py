...
class PaperTrader:
    ...
    async def _can_open_position(self) -> bool:
        """Check if we can open new positions."""
        # Check max positions
        if len(self.open_positions) >= self.TRADING_RULES['entry_conditions']['max_open_positions']:
            return False
        
        # Check daily loss limit
        if self.daily_pnl <= self.TRADING_RULES['entry_conditions']['no_new_trades_if_daily_loss']:
            return False
    ...
    async def _get_trading_signals(self) -> List[Dict]:
        ...
        for symbol in symbols:
            ...
            if prediction and prediction['confidence'] >= self.TRADING_RULES['entry_conditions']['ml_confidence_minimum']:
    ...
    async def _process_signal(self, signal: Dict):
        ...
        try:
            ...
            stop_loss_price = current_price * (1 - self.TRADING_RULES['exit_conditions']['stop_loss'] / 100)
            take_profit_price = current_price * (1 + self.TRADING_RULES['exit_conditions']['take_profit'] / 100)
