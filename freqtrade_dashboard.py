#!/usr/bin/env python3
"""
Freqtrade-integrated trading dashboard (exact copy of live_dashboard_v2 UI)
"""
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // No auto-refresh on admin panel to prevent losing unsaved changes
...
def get_engine_status():
    """Check if Freqtrade engine is running"""
    try:
        # Check for recent Freqtrade scan activity
        db = SupabaseClient()
        
        # Freqtrade on Railway scans periodically, check for scans within last 10 minutes
        ten_minutes_ago = (
            datetime.now(timezone.utc) - timedelta(minutes=10)
        ).isoformat()

        # Check for recent scan history from Freqtrade - get the most recent one
        result = (
            db.client.table("scan_history")
            .select("timestamp")
            .eq("strategy_name", "CHANNEL")  # Freqtrade uses CHANNEL strategy
            .order("timestamp", desc=True)
            .limit(1)
            .execute()
        )

        # Check if the scan is recent (within 10 minutes)
        ...
        if result.data:
            ...
            scan_time = dt.fromisoformat(last_activity.replace('Z', '+00:00'))
            time_diff = datetime.now(timezone.utc) - scan_time
            # Consider running if scanned within last 10 minutes
            freqtrade_running = time_diff.total_seconds() < 600
            
        return jsonify({
            "running": freqtrade_running, 
            "source": "freqtrade",
