...
class FeatureAnalyzer:
    ...
    def load_data(self):
        ...
        exclude_cols = ['symbol', 'timestamp', 'outcome', 'outcome_binary', 
                       'setup_price', 'high_4h', 'exit_price', 'exit_timestamp',
                       'pnl_pct', 'take_profit_target', 'stop_loss_target']
    ...
    def analyze_by_symbol_characteristics(self):
        ...
        symbol_stats.columns = ['win_rate', 'count', 'tier', 'avg_volatility', 'vs_btc', 'correlation']
        ...
        for symbol, row in best.iterrows():
            logger.info(f"  {symbol:6s}: {row['win_rate']:.1%} win rate ({int(row['count'])} setups) - {row['tier']} cap")
