...
async def test_full_flow():
    ...
    try:
        ...
        if setup:
            logger.info(
                f"✅ SwingDetector setup has position_size_multiplier: {setup.get('position_size_multiplier', 'MISSING')}"
            )
...
async def check_paper_trading_config():
    ...
    try:
        ...
        logger.info(
            f"Config has position_size_multiplier: {PAPER_TRADING_CONFIG.get('position_size_multiplier', 'NOT FOUND')}"
        )
