...
class HummingbotAPITest:
    ...
    async def test_authentication(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(
                f"{self.base_url}/api/v1/accounts", auth=auth
            ) as response:
                if response.status == 200:
    ...
    async def test_paper_trading_setup(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            bot_config = {
                ...
                "initial_balance": {"USDT": 100000, "BTC": 0, "ETH": 0},
            }

            async with session.post(
                f"{self.base_url}/bots", json=bot_config, auth=auth
            ) as response:
                if response.status == 200:
    ...
    async def test_order_placement(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for endpoint in order_endpoints:
                async with session.post(
                    f"{self.base_url}{endpoint}", json=order, auth=auth
                ) as response:
                    if response.status == 200:
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            logger.info("\n📝 NEXT STEPS:")
            logger.info(
                "1. Check http://localhost:8000/docs for full API documentation"
            )
