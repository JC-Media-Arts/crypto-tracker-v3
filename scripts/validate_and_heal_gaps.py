...
class GapDetector:
    ...
    def __init__(self):
        ...
        self.expected_intervals = {"1m": 1, "15m": 15, "1h": 60, "1d": 1440}
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["scan", "heal", "report", "completeness"],
        default="scan",
        ...
        for symbol, stats in results.items():
            ...
            for tf, data in stats.items():
                if "completeness_pct" in data:
                    print(
                        f"  {tf}: {data['completeness_pct']}% complete ({data['actual_bars']}/{data['expected_bars']} bars)"
                    )
