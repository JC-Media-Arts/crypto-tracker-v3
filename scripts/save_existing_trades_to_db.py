    ...
    try:
        ...
        if state_file.exists():
            ...
            for symbol, position in state.get('positions', {}).items():
                try:
                    ...
                    existing = db.client.table("paper_trades").select("*").eq("symbol", symbol).eq("status", "FILLED").eq("trading_engine", "simple_paper_trader").execute()
        ...
        if trades_file.exists():
            ...
            for trade in trades:
                try:
                    # Check if already exists
                    existing = db.client.table("paper_trades").select("*").eq("symbol", trade['symbol']).eq("side", "SELL").eq("trading_engine", "simple_paper_trader").execute()
        ...
        try:
            ...
            existing = db.client.table("paper_performance").select("*").eq("date", today).eq("trading_engine", "simple_paper_trader").execute()
            ...
            if not existing.data:
                ...
                db.client.table("paper_performance").update(perf_data).eq("date", today).eq("trading_engine", "simple_paper_trader").execute()
