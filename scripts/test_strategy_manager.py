...
class StrategyManagerTest:
    ...
    async def test_opportunity_scanning(self):
        ...
        for signal in signals:
            logger.info(
                f"  {signal.symbol} - {signal.strategy_type.value}: "
    ...
    async def test_capital_constraints(self):
        ...
        logger.info(
            f"  DCA: ${status['capital_allocation']['dca_available']:.2f} available "
            f"(of ${self.config['total_capital'] * 0.6:.2f})"
        )
        logger.info(
            f"  Swing: ${status['capital_allocation']['swing_available']:.2f} available "
