...
def main():
    ...
    while True:
        ...
        ).order('symbol', desc=False).order('timestamp', desc=False).limit(limit).offset(offset)
    ...
    for symbol in sorted(df['symbol'].unique()):
        ...
        stats_dict = {
            ...
            'first_timestamp': symbol_data.iloc[0]['timestamp'].strftime('%Y-%m-%d %H:%M'),
            'last_timestamp': symbol_data.iloc[-1]['timestamp'].strftime('%Y-%m-%d %H:%M'),
            ...
            'price_range': (symbol_data['high'].max() - symbol_data['low'].min()) / first_price * 100
        }
    ...
    stats_file = f"data/market_stats_48hr_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    while True:
        ...
        ).order('created_at', desc=False).limit(limit).offset(offset)
    ...
    if trades_result.data:
        ...
        # Save trades to CSV
        trades_file = f"data/paper_trades_48hr_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        ...
        if not open_trades.empty:
            ...
            print(f"  Total P&L: ${(realized_pnl if 'realized_pnl' in locals() else 0) + unrealized_pnl:.2f}")
...
"""
    ...
    for symbol in ['BTC', 'ETH', 'SOL']:
        ...
        if not symbol_data.empty:
            ...
            if largest_drop < -2:
                drop_time = symbol_data[symbol_data['pct_change'] == largest_drop]['timestamp'].iloc[0]
...
"""
    ...
    # Save markdown file
    markdown_file = f"data/market_analysis_48hr_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
