...
class ShadowDashboard:
    ...
    async def generate_layout(self) -> Layout:
        ...
        layout["performance"].split_column(
            Layout(name="champion_vs_challengers"),
            Layout(name="recent_outcomes")
        )
    ...
    async def _create_performance_table(self) -> Panel:
        ...
        table = Table(title="Champion vs Challengers (7-day)", show_header=True, header_style="bold magenta")
        ...
        try:
            ...
            result = self.supabase.client.table('shadow_performance')\
                .select('*')\
                .eq('timeframe', '7d')\
    ...
    async def _create_outcomes_table(self) -> Panel:
        ...
        table = Table(title="Recent Shadow Outcomes (Last 24h)", show_header=True, header_style="bold magenta")
        ...
        try:
            ...
            result = self.supabase.client.table('shadow_outcomes')\
        ...
        try:
            ...
            if recommendations:
                ...
                for i, rec in enumerate(recommendations[:3], 1):
                    ...
                    text.append(f"{i}. {rec.strategy_name} - {rec.parameter_name}\n", style="bold cyan")
                    text.append(f"   {conf_emoji} Confidence: {rec.confidence_level}\n", style="white")
                    text.append(f"   Current: {rec.current_value:.2f}\n", style="dim")
                    text.append(f"   Recommended: {rec.recommended_value:.2f}\n", style="bold green")
            ...
            return Panel("Error loading recommendations", title="Recommendations", border_style="red")
    ...
    async def _print_statistics(self):
        ...
        try:
            ...
            if result.data:
                ...
                stats_table = Table(title="24-Hour Statistics", show_header=True, header_style="bold magenta")
