    ...
    def process_symbol(self, symbol: str, lookback_days: int) -> List[Dict]:
        ...
        for setup in setups:
            label = self.simulate_outcome(
                df, setup,
                tier_config['take_profit'],
                tier_config['stop_loss']
            )
    ...
    def generate_all_labels(self, lookback_days: int = 180) -> pd.DataFrame:
        ...
        if all_labels:
            ...
            df['market_cap_tier'] = df['symbol'].apply(
                lambda s: 'large' if s in self.market_tiers['large_cap']['symbols']
                else 'mid' if s in self.market_tiers['mid_cap']['symbols']
                else 'small'
            )
