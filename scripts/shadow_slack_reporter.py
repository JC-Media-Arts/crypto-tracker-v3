...
class ShadowSlackReporter:
    ...
    async def send_daily_summary(self):
        ...
        try:
            ...
            if performance['champion_win_rate'] > 0:
                message = (f"Champion Win Rate: {performance['champion_win_rate']:.1%} | "
        ...
        try:
            ...
            if successful:
                ...
                for result in successful:
                    param_detail = (f"{result.old_value:.3f} → {result.new_value:.3f} "
    ...
    async def send_rollback_alert(self, adjustment_id: int, reason: str):
        ...
        try:
            ...
            result = self.supabase.client.table('threshold_adjustments')\
                .select('*')\
            ...
            if result.data:
                ...
                await self.slack.send_notification(
                    NotificationType.ROLLBACK_ALERT,
                    title,
                    message,
                    details,
                    "danger"
    ...
    async def _get_performance_summary(self) -> Dict:
        ...
        try:
            ...
            champion_result = self.supabase.client.table('shadow_performance')\
                .select('*')\
            ...
            # Get best challenger
            challenger_result = self.supabase.client.table('shadow_performance')\
                .select('*')\
                .eq('strategy_name', 'OVERALL')\
                .eq('timeframe', '24h')\
            ...
            # Count total shadows
            shadow_count = self.supabase.client.table('shadow_outcomes')\
                .select('outcome_id', count='exact')\
                .gte('evaluated_at', (datetime.utcnow() - timedelta(hours=24)).isoformat())\
                .execute()
    ...
    def _format_champion_stats(self, performance: Dict) -> str:
        ...
        if performance.get('champion_trades', 0) > 0:
            return (f"Win Rate: {performance['champion_win_rate']:.1%} | "
    ...
    def _format_top_performers(self, performers: List) -> str:
        ...
        for i, perf in enumerate(performers[:3], 1):
            emoji = ['🥇', '🥈', '🥉'][i-1]
            lines.append(f"{emoji} {perf.variation_name}: "
    ...
    def _format_recommendations(self, recommendations: List) -> str:
        ...
        for rec in recommendations[:3]:
            ...
            lines.append(f"{conf_emoji} {rec.strategy_name}.{rec.parameter_name}: "
    ...
    def _format_adjustments(self, adjustments: List[Dict]) -> str:
        ...
        for adj in adjustments[:3]:
            ...
            lines.append(f"{status} {adj['strategy_name']}.{adj['parameter_name']}: "
