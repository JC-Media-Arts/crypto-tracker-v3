...
def analyze_targets():
    ...
    df['drop_bucket'] = pd.cut(df['drop_pct'], bins=[-100, -10, -7, -5, 0], labels=['Huge (>10%)', 'Large (7-10%)', 'Medium (5-7%)', 'Small (<5%)'])
    ...
    df['rsi_bucket'] = pd.cut(df['rsi'], bins=[0, 30, 50, 70, 100], labels=['Oversold (<30)', 'Low (30-50)', 'Neutral (50-70)', 'High (>70)'])
