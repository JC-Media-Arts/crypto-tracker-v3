...
def analyze_btc_movement(df: pd.DataFrame) -> Dict:
    ...
    btc_data['rolling_std'] = btc_data['pct_change'].rolling(4).std()  # 4-hour rolling volatility
    ...
    btc_data['drawdown'] = (btc_data['close'] - btc_data['cummax']) / btc_data['cummax'] * 100
...
def analyze_paper_trades(supabase, start_time: datetime, end_time: datetime) -> Dict:
    ...
    # Analyze by status
    status_counts = trades_df['status'].value_counts() if 'status' in trades_df.columns else pd.Series()
    
    # Analyze timing
    hourly_trades = trades_df.set_index('created_at').resample('1H').agg({
        'trade_id': 'count',
        'pnl': 'mean'
    }) if 'pnl' in trades_df.columns else trades_df.set_index('created_at').resample('1H').size()
    
    # Find trades during major market moves
    closed_trades = trades_df[trades_df['status'] == 'CLOSED'].copy() if 'status' in trades_df.columns else pd.DataFrame()
...
def generate_insights(btc_analysis: Dict, market_corr: pd.DataFrame, 
                      paper_trades: Dict, market_regime: str) -> List[str]:
    """Generate actionable insights from the analysis."""
    insights = []
    
    # Market structure insights
    if "FLASH CRASH" in market_regime or "SHARP CORRECTION" in market_regime:
        insights.append("🚨 MAJOR FINDING: System experienced a flash crash event with rapid price decline")
        insights.append("   → Need better crash detection and emergency exit mechanisms")
        insights.append("   → Consider implementing circuit breakers for extreme moves")
    
    if "V-SHAPED RECOVERY" in market_regime:
        insights.append("📈 OPPORTUNITY: V-shaped recovery detected - system should capture these bounces")
        insights.append("   → Implement mean reversion strategies for oversold conditions")
    ...
    if not market_corr.empty:
        ...
        if not high_beta_coins.empty:
            insights.append(f"🔗 CORRELATION: {len(high_beta_coins)} coins showed high beta (>1.5) to BTC")
            insights.append("   → These coins amplify BTC moves - adjust position sizes accordingly")
            insights.append("   → Consider reducing exposure to high-beta coins during crashes")
    
    # Trading performance insights
    if paper_trades and 'strategy_performance' in paper_trades and not paper_trades['strategy_performance'].empty:
        ...
        for strategy in paper_trades['strategy_performance'].index:
            if ('trade_id', 'count') in paper_trades['strategy_performance'].columns:
                count = paper_trades['strategy_performance'].loc[strategy, ('trade_id', 'count')]
                if ('pnl', 'mean') in paper_trades['strategy_performance'].columns:
                    profit = paper_trades['strategy_performance'].loc[strategy, ('pnl', 'mean')]
                    insights.append(f"   {strategy}: {count} trades, avg PnL: ${profit:.2f}")
                else:
                    insights.append(f"   {strategy}: {count} trades opened (not yet closed)")
    ...
    if btc_analysis['largest_hourly_drop'] < -1:
        ...
        insights.append("   → Consider time-based filters or reduced trading during volatile hours")
...
def main():
    ...
    if btc_analysis:
        ...
        print(f"   Occurred at:  {btc_analysis['max_drawdown_time'].strftime('%H:%M:%S')}")
        print(f"   Lowest Price: ${btc_analysis['min_price']:,.2f} at {btc_analysis['min_price_time'].strftime('%H:%M:%S')}")
        
        print(f"\n⚡ Volatility Metrics:")
        print(f"   Price Range:     {btc_analysis['price_range_pct']:.2f}% of starting price")
        print(f"   Avg Volatility:  {btc_analysis['avg_volatility']:.3f}%")
        print(f"   Max Volatility:  {btc_analysis['max_volatility']:.3f}%")
        
        print(f"\n🎢 Extreme Moves:")
        print(f"   Largest Hourly Drop: {btc_analysis['largest_hourly_drop']:.2f}% at {btc_analysis['largest_hourly_drop_time'].strftime('%H:%M:%S')}")
        print(f"   Largest Hourly Gain: {btc_analysis['largest_hourly_gain']:.2f}% at {btc_analysis['largest_hourly_gain_time'].strftime('%H:%M:%S')}")
    ...
    if not market_corr.empty:
        ...
        for _, row in top_5.iterrows():
            print(f"   {row['symbol']:12} Return: {row['total_return']:+6.2f}%  "
        ...
        print(f"   Winners/Losers:      {(market_corr['total_return'] > 0).sum()}/{(market_corr['total_return'] <= 0).sum()}")
    ...
    report = {
        ...
        'market_stats': {
            ...
            'avg_market_return': float(market_corr['total_return'].mean()) if not market_corr.empty else 0,
            'avg_btc_correlation': float(market_corr['btc_correlation'].mean()) if not market_corr.empty else 0
        },
        'paper_trading': {
    ...
    report_file = f"data/market_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def create_visualization(btc_data: pd.DataFrame, market_corr: pd.DataFrame, paper_trades: Dict):
    ...
    fig = make_subplots(
        rows=3, cols=2,
        subplot_titles=(
            'BTC Price Movement', 'BTC Returns Distribution',
            'Market Drawdowns', 'Correlation with BTC',
            'Hourly Volatility', 'Paper Trading Activity'
        ),
        specs=[
            [{'secondary_y': True}, {}],
            [{}, {}],
