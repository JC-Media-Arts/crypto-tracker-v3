    ...
    try:
        ...
        if recent_scans.data:
            ...
            for scan in recent_scans.data[:5]:
                logger.info(f"   {scan['timestamp']}: {scan['strategy']} - {scan['symbol']} - {scan.get('decision', 'N/A')}")
        else:
    ...
    try:
        ...
        if recent_trades.data:
            ...
            for trade in recent_trades.data[:5]:
                logger.info(f"   {trade['timestamp']}: {trade['symbol']} - {trade['action']} - ${trade.get('amount', 'N/A')}")
        else:
