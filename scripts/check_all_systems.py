...
class SystemStatusChecker:
    ...
    def check_paper_trading(self):
        ...
        try:
            result = self.supabase.client.table("trade_logs") \
                .select("*", count="exact") \
                .eq("status", "OPEN") \
                .execute()
    ...
    def check_ml_engine(self):
        ...
        try:
            result = self.supabase.client.table("ml_features") \
                .select("timestamp") \
    ...
    def check_shadow_testing(self):
        ...
        for table, time_col in shadow_tables:
            try:
                ...
                result = self.supabase.client.table(table) \
                    .select("*", count="exact") \
                    .execute()
    ...
    def check_data_pipeline(self):
        ...
        ws_running = self.check_process("websocket") or self.check_process("data_collector") or self.check_process("singleton_websocket")
    ...
    def check_strategy_engines(self):
        ...
        for strategy in strategies:
            ...
            try:
                one_hour_ago = (datetime.now(timezone.utc) - timedelta(hours=1)).isoformat()
                result = self.supabase.client.table("scan_history") \
                    .select("*", count="exact") \
            ...
            try:
                ...
                result = self.supabase.client.table(strategy_table) \
                    .select("*", count="exact") \
                    .execute()
    ...
    def check_recent_table_activity(self, table_name: str, time_column: str, minutes: int) -> int:
        ...
        try:
            ...
            result = self.supabase.client.table(table_name) \
                .select("*", count="exact") \
    ...
    def print_component_status(self, component: str, checks: Dict[str, Any]):
        ...
        if not critical_checks:
            ...
            all_good = all(
                (isinstance(v, str) and "Error" not in v and "not found" not in v.lower()) or 
    ...
    def print_summary(self):
        ...
        critical_systems = ["Paper Trading", "ML Engine", "Data Pipeline", "Risk Management"]
        critical_ok = 0
        ...
        for system in critical_systems:
            if system in self.results:
                ...
                has_positive = any(
                    ...
                    (isinstance(v, str) and "Error" not in v and "No " not in v) or
                    (isinstance(v, (int, float)) and v > 0)
                    for v in checks.values()
                )
                if has_positive:
        ...
        for component, checks in self.results.items():
            positive_checks = [
                ...
                if (isinstance(v, bool) and v) or
                   ...
                   (isinstance(v, str) and "Error" not in v and "not found" not in v.lower() and v != "0")
            ]
            if positive_checks:
