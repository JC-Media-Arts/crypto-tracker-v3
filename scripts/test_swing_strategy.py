...
class SwingStrategyTest:
    ...
    def validate_configuration(self):
        ...
        tests = [
            ('breakout_threshold', config.get('breakout_threshold', 0) * 100, self.expected_config['breakout_threshold']),
            ('volume_spike_threshold', config.get('volume_spike_threshold', 0), self.expected_config['volume_surge']),
            ('rsi_bullish_min', config.get('rsi_bullish_min', 0), 50),  # Should be above 50
            ('min_price_change_24h', config.get('min_price_change_24h', 0), self.expected_config['breakout_threshold']),
        ]
    ...
    def test_exit_strategies(self):
        ...
        for pos in test_positions:
            ...
            logger.info(f"  {'✅' if tp_passed else '❌'} Take Profit: ${take_profit:,.2f} (Expected: ~${pos['expected_tp']:,.2f})")
            logger.info(f"  {'✅' if sl_passed else '❌'} Stop Loss: ${stop_loss:,.2f} (Expected: ~${pos['expected_sl']:,.2f})")
        ...
        for symbol in test_symbols:
            ...
            if ohlc_data and len(ohlc_data) > 50:
                ...
                if setup:
                    ...
                    logger.info(f"  TP: ${setup['take_profit']:,.2f} (+{((setup['take_profit']/setup['entry_price'])-1)*100:.1f}%)")
                    logger.info(f"  SL: ${setup['stop_loss']:,.2f} (-{(1-(setup['stop_loss']/setup['entry_price']))*100:.1f}%)")
    ...
    async def _fetch_real_data(self, symbol):
        ...
        try:
            response = self.supabase.client.table('unified_ohlc').select('*').eq(
                'symbol', symbol
            ).eq(
                'timeframe', '1h'
            ).order(
