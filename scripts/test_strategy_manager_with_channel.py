    ...
    async def test_all_strategies(self):
        ...
        for result in results:
            if result['success']:
                ...
                logger.warning(f"  {result['symbol']} - {result['strategy']} - FAILED: {result.get('error', 'Unknown')}")
        ...
        # Show capital usage
        logger.info(f"\n💼 Capital Usage:")
        logger.info(f"  DCA used: ${self.manager.allocation.dca_used:.2f} / ${self.config['total_capital'] * self.config['dca_allocation']:.0f}")
        logger.info(f"  Swing used: ${self.manager.allocation.swing_used:.2f} / ${self.config['total_capital'] * self.config['swing_allocation']:.0f}")
        logger.info(f"  Channel used: ${self.manager.allocation.channel_used:.2f} / ${self.config['total_capital'] * self.config['channel_allocation']:.0f}")
