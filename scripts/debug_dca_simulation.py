...
def analyze_dca_simulation():
    ...
    for _, row in worst_drawdowns.iterrows():
        print(f"  {row['symbol']} on {row['setup_time'][:10]}: {row['max_drawdown']:.2f}% -> {row['label']} ({row['pnl_pct']:.2f}%)")
    ...
    test_setup = labels_df[labels_df['max_drawdown'] < -5].iloc[0] if any(labels_df['max_drawdown'] < -5) else labels_df.iloc[0]
