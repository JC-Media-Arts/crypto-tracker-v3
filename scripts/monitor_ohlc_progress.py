...
def main():
    ...
    for tf in timeframes:
        ...
        result = supabase.client.rpc(
            'get_unique_symbols_by_timeframe',
            {'tf': tf}
        ).execute() if False else None  # RPC might not exist
        
        # Fallback: direct query
        try:
            result = supabase.client.table('ohlc_data').select(
                'symbol'
            ).eq('timeframe', tf).execute()
    ...
    for tf in timeframes:
        try:
            result = supabase.client.table('ohlc_data').select(
                'symbol', count='exact'
            ).eq('timeframe', tf).limit(1).execute()
            
            # This is a workaround - we need to query without limit to get true count
            # But for now, let's get sample data
            sample = supabase.client.table('ohlc_data').select(
                'symbol,timestamp'
            ).eq('timeframe', tf).limit(5000).execute()
