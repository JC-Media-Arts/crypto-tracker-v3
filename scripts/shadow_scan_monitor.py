...
class ShadowScanMonitor:
    ...
    async def get_unprocessed_scans(self):
        ...
        try:
            ...
            scans_result = self.supabase_client.client.table("scan_history").select("*").gte("timestamp", cutoff).order("scan_id", desc=False).execute()
            ...
            shadows_result = self.supabase_client.client.table("shadow_variations").select("scan_id").in_("scan_id", scan_ids).execute()
