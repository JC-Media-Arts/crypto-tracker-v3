...
class SimpleStrategyManager:
    ...
    def resolve_conflicts(self, signals: List[StrategySignal]) -> List[StrategySignal]:
        ...
        for symbol, sigs in symbol_signals.items():
            if len(sigs) == 1:
                ...
                logger.info(f"Conflict for {symbol}: {len(sigs)} signals, "
        ...
        for signal in signals:
            if signal.strategy_type == StrategyType.DCA:
                ...
                if signal.required_capital <= available:
                    ...
                    logger.info(f"Approved DCA for {signal.symbol}: ${signal.required_capital:.0f} "
                               f"(${available - signal.required_capital:.0f} remaining)")
                else:
                    logger.warning(f"Rejected DCA for {signal.symbol}: needs ${signal.required_capital:.0f}, "
                ...
                if signal.required_capital <= available:
                    ...
                    logger.info(f"Approved Swing for {signal.symbol}: ${signal.required_capital:.0f} "
                               f"(${available - signal.required_capital:.0f} remaining)")
                else:
                    logger.warning(f"Rejected Swing for {signal.symbol}: needs ${signal.required_capital:.0f}, "
                                 f"only ${available:.0f} available")
    ...
    def execute_signals(self, signals: List[StrategySignal]) -> Dict:
        ...
        for signal in signals:
            if signal.symbol in self.active_positions:
                ...
                logger.info(f"Executed {signal.strategy_type.value} for {signal.symbol}: "
    ...
    def update_performance(self, symbol: str, pnl: float, is_win: bool):
        ...
        if symbol in self.active_positions:
            ...
            logger.info(f"{strategy.value} trade closed for {symbol}: "
...
def test_orchestration():
    ...
    for sig in signals:
        logger.info(f"  {sig.symbol} - {sig.strategy_type.value}: "
    ...
    logger.info(f"  DCA: ${status['capital']['dca_used']:.0f} used of ${status['capital']['dca_allocated']:.0f} "
               f"(${status['capital']['dca_available']:.0f} available)")
    logger.info(f"  Swing: ${status['capital']['swing_used']:.0f} used of ${status['capital']['swing_allocated']:.0f} "
