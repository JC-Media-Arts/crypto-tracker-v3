...
class PaperTradingSystem:
    ...
    async def get_current_prices(self, symbols: List[str]) -> Dict[str, float]:
        ...
        for symbol in symbols:
            try:
                result = await self.data_fetcher.get_recent_data(
                    symbol=symbol,
                    hours=1,  # Get last hour for current price
                    timeframe="1m"
                )
    ...
    async def check_and_close_positions(self, market_data: Dict) -> None:
        ...
        closed_trades = await self.paper_trader.check_and_close_positions(
            prices,
            max_hold_hours=self.max_position_duration_hours  # 72 hours
        )
        
        # Log closed trades
        for trade in closed_trades:
            emoji = "✅" if trade.pnl_usd > 0 else "❌"
            logger.info(f"{emoji} Closed {trade.symbol}: ${trade.pnl_usd:.2f} ({trade.pnl_percent:+.2f}%) - {trade.exit_reason}")
    ...
    async def run_trading_loop(self) -> None:
        ...
        while not self.shutdown:
            try:
                ...
                logger.info(f"Scan complete: {len(signals)} signals, {stats['positions']}/{stats['max_positions']} positions")
                ...
                if self.paper_trader.positions:
                    ...
                    for symbol, position in self.paper_trader.positions.items():
                        ...
                        logger.info(f"   {emoji} {symbol}: Entry ${position.entry_price:.4f} → ${current_price:.4f} ({pnl_pct:+.2f}%){trailing_info}")
...
async def main():
    ...
    for sig in (signal.SIGTERM, signal.SIGINT):
        loop.add_signal_handler(
            sig,
            lambda: asyncio.create_task(system.shutdown_handler())
        )
