...
class ComprehensiveTradingTest:
    ...
    async def test_dca_detection(self):
        ...
        for scenario in test_scenarios:
            ...
            logger.info(f"{result} - {scenario['name']}: "
                       f"RSI={scenario['rsi']}, "
        ...
        for symbol in test_symbols:
            ...
            if ohlc_data and len(ohlc_data) > 50:
                ...
                if predictions:
                    ...
                    logger.info(f"  Position Size Multiplier: {predictions['position_size_multiplier']:.2f}x")
                    logger.info(f"  Take Profit: {predictions['take_profit_percent']:.2f}%")
                    logger.info(f"  Stop Loss: {predictions['stop_loss_percent']:.2f}%")
                    logger.info(f"  Expected Hold Time: {predictions['expected_hold_hours']:.1f} hours")
                    logger.info(f"  Win Probability: {predictions['win_probability']:.2%}")
                    logger.info(f"  Confidence Score: {predictions['confidence_score']:.2%}")
    ...
    async def test_position_sizing(self):
        ...
        for case in test_cases:
            ...
            logger.info(f"  → Position Size: ${position_size:,.2f} ({risk_percent:.2f}% of portfolio)")
    ...
    async def test_grid_generation(self):
        ...
        for grid_params in test_grids:
            ...
            for i, order in enumerate(grid['orders'], 1):
                logger.info(f"    Order {i}: ${order['price']:,.2f} x {order['size']:.6f} = ${order['value']:,.2f}")
    ...
    async def test_order_execution(self):
        ...
        if placed_orders:
            ...
            cancelled = await self.hummingbot_client.cancel_order(
                self.bot_id, 
                order_to_cancel.get('id')
            )
            
            if cancelled:
                logger.info(f"✅ Successfully cancelled order: {order_to_cancel.get('id')}")
            else:
    ...
    async def test_exit_strategies(self):
        ...
        for position in test_positions:
            ...
            logger.info(f"  {result} - Action: {action} (Expected: {position['expected_action']})")
        ...
        for scenario in risk_scenarios:
            ...
            for position in scenario['positions']:
                ...
                if position_percent > 10:
                    warnings.append(f"{position['symbol']} exceeds 10% limit ({position_percent:.1f}%)")
                    
            # Check total exposure
            if exposure_percent > 30:
                warnings.append(f"Total exposure exceeds 30% limit ({exposure_percent:.1f}%)")
            ...
            if len(scenario['positions']) >= 2:
                ...
                if 'BTC' in symbols and 'ETH' in symbols:
                    btc_value = next(p['value'] for p in scenario['positions'] if p['symbol'] == 'BTC')
                    eth_value = next(p['value'] for p in scenario['positions'] if p['symbol'] == 'ETH')
                    if (btc_value + eth_value) / portfolio_value > 0.15:
                        warnings.append("High correlation risk: BTC + ETH > 15%")
                        
            has_warnings = len(warnings) > 0
            result = "✅ PASS" if (has_warnings == scenario['expected_warning']) else "❌ FAIL"
            
            logger.info(f"  Total Exposure: ${total_exposure:,.2f} ({exposure_percent:.1f}%)")
            if warnings:
            ...
            logger.info(f"  {result} - Expected Warning: {scenario['expected_warning']}, Got: {has_warnings}")
        ...
        if self.bot_id:
            ...
            if positions:
                ...
                for pos in positions:
                    logger.info(f"  {pos.symbol}: {pos.amount} @ ${pos.entry_price:.2f}, "
                              f"PnL: ${pos.pnl:.2f}")
            else:
                logger.info("\nNo active positions")
    ...
    def _create_mock_ohlc_data(self, symbol, rsi, price_drop_1h, price_drop_24h, volume_spike):
    ...
    def _validate_ml_predictions(self, symbol, predictions):
        ...
        if not 0.5 <= predictions['position_size_multiplier'] <= 3.0:
            validations.append(f"Position multiplier out of range: {predictions['position_size_multiplier']}")
            
        # Check take profit
        if not 2.0 <= predictions['take_profit_percent'] <= 20.0:
            validations.append(f"Take profit out of range: {predictions['take_profit_percent']}")
            
        # Check stop loss
        if not 1.0 <= predictions['stop_loss_percent'] <= 10.0:
            validations.append(f"Stop loss out of range: {predictions['stop_loss_percent']}")
            
        # Check win probability
        if not 0.3 <= predictions['win_probability'] <= 0.9:
            validations.append(f"Win probability out of range: {predictions['win_probability']}")
    ...
    def _validate_grid(self, params, grid):
        ...
        if abs(total_value - params['total_investment']) > 1:
            logger.warning(f"  ⚠️  Grid total mismatch: ${total_value:.2f} vs ${params['total_investment']:.2f}")
        else:
        ...
        if len(prices) > 1:
            spacing = [(prices[i] - prices[i+1]) / prices[i+1] * 100 
    ...
    async def generate_report(self):
        ...
        if total_tests > 0:
            ...
            if overall_pass_rate == 100:
                ...
                logger.info("✅ System mostly functional. Review warnings before live trading.")
            else:
