...
class SimplifiedTradingSystem:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            "dca_config": self.recovery_config.get("dca_config", {
                "drop_threshold": -3.5,
                "min_confidence": 0.0,
                "use_ml": False
            }),
            "swing_config": self.recovery_config.get("swing_config", {
                "breakout_threshold": 2.1,
                "min_confidence": 0.0,
                "use_ml": False
            }),
            "channel_config": self.recovery_config.get("channel_config", {
                "min_channel_strength": 0.42,
                "min_confidence": 0.0,
                "use_ml": False
            }),
            
            # Trading rules from recovery config
            "trading_rules": self.recovery_config.get("trading_rules", {
    ...
    async def check_exits(self, market_data: Dict):
        ...
        for symbol, position in self.active_positions.items():
            try:
                ...
                # Time exit
                elif (datetime.now() - position["entry_time"]).total_seconds() / 3600 > self.config["trading_rules"]["time_exit_hours"]:
                    should_exit = True
