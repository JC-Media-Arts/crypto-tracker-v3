...
class SystemReview:
    ...
    async def check_database_performance(self):
        ...
        checks = [
            (
                "Materialized view 'ohlc_today' exists",
                self.verify_view_exists("ohlc_today"),
            ),
            (
                "Materialized view 'ohlc_recent' exists",
                self.verify_view_exists("ohlc_recent"),
            ),
    ...
    async def check_symbol_coverage(self) -> bool:
        ...
        try:
            ...
            result = self.db.client.rpc(
                "execute_sql",
                {"query": "SELECT COUNT(DISTINCT symbol) as count FROM ohlc_recent"},
            ).execute()
    ...
    async def test_hybrid_fetcher(self) -> bool:
        ...
        try:
            ...
            return all(
                [
                    latest is not None,
                    recent and len(recent) > 0,
                    ml_data and ml_data.get("has_data", False),
                ]
            )
        except Exception:
            return False
