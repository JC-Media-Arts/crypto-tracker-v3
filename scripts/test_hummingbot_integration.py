...
class MockSupabaseClient:
    ...
    def execute(self):
        return type(
            "obj", (object,), {"data": self.data_store.get(self.current_table, [])}
        )()
...
async def test_hummingbot_integration():
    ...
    print(
        f"   Created signal for {test_signal['symbol']} with {test_signal['confidence']:.0%} confidence"
    )
    ...
    if predictions:
        ...
        for pred in predictions:
            print(
                f"   - {pred['symbol']}: {pred['prediction']} (confidence: {pred['confidence']:.1%})"
            )
    else:
