        ...
        try:
            ...
            if count > 0:
                ...
                try:
                    ...
                    oldest_result = supabase.client.table("ohlc_data")\
                        .select("timestamp")\
                        .eq("timeframe", tf)\
                        .order("timestamp")\
                        .limit(1)\
                ...
                # Get newest
                try:
                    newest_result = supabase.client.table("ohlc_data")\
                        .select("timestamp")\
                        .eq("timeframe", tf)\
                ...
                try:
                    ...
                    recent_result = supabase.client.table("ohlc_data")\
                        .select("timestamp", count="exact")\
                        .eq("timeframe", tf)\
                ...
                try:
                    ...
                    month_result = supabase.client.table("ohlc_data")\
                        .select("timestamp", count="exact")\
                        .eq("timeframe", tf)\
    ...
    try:
        ...
        for tf in ['1min', '1m']:
            ...
            if count > 0:
                ...
                # Get unique symbols
                symbols_result = supabase.client.table("ohlc_data")\
                    .select("symbol")\
                    .eq("timeframe", tf)\
                    .gte("timestamp", one_hour_ago)\
                    .execute()
