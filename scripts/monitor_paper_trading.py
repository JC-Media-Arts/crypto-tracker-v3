...
class PaperTradingMonitor:
    ...
    def get_stats_panel(self) -> Panel:
        ...
        # Row 2
        table.add_row(
            "📊 Positions", f"{stats['positions']}/{stats['max_positions']}",
            "💵 Position Value", f"${stats['positions_value']:.2f}"
        )
        
        # Row 3
        table.add_row(
            "💹 Total P&L", Text(f"${stats['total_pnl']:+.2f} ({stats['total_pnl_pct']:+.2f}%)", style=pnl_color),
            "🎯 Win Rate", Text(f"{stats['win_rate']:.1f}%", style=win_rate_color)
        )
        ...
        # Row 5
        table.add_row(
            "💸 Fees Paid", f"${stats['total_fees']:.2f}",
            "📉 Slippage", f"${stats['total_slippage']:.2f}"
        )
