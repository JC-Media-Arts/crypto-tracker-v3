...
class IncrementalOHLCUpdater:
    ...
    def get_latest_timestamp(self, symbol: str, timeframe: str) -> Optional[datetime]:
        ...
        try:
            response = self.supabase.client.table('ohlc_data').select('timestamp').eq(
                'symbol', symbol
            ).eq(
                'timeframe', timeframe
            ).order(
    ...
    def fetch_ohlc_from_polygon(self, symbol: str, timeframe: str, from_date: datetime, to_date: datetime) -> List[Dict]:
    ...
    def save_ohlc_batch(self, data: List[Dict], symbol: str, timeframe: str) -> int:
        ...
        try:
            ...
            response = self.supabase.client.table('ohlc_data').upsert(
                records,
...
def main():
    ...
    parser.add_argument('--timeframe', choices=['1m', '15m', '1h', '1d', 'all'],
                       default='all', help='Timeframe to update')
