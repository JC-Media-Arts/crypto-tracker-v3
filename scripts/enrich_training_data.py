...
class TrainingDataEnricher:
    ...
    def calculate_symbol_context(self, symbol: str, timestamp: datetime) -> Dict:
        ...
        # Get symbol data
        symbol_result = self.supabase.client.table("ohlc_data").select(
            "timestamp,close"
        ).eq(
            "symbol", symbol
        ).eq(
    ...
    def enrich_training_data(self, input_file: str, output_file: str):
        ...
        for i in range(0, len(df), batch_size):
            ...
            for idx, row in batch.iterrows():
                timestamp = row['timestamp'].replace(tzinfo=tz.UTC) if row['timestamp'].tzinfo is None else row['timestamp']
                symbol = row['symbol']
                ...
                enriched_row = {**row.to_dict(), **btc_metrics, **volatility_metrics, **symbol_context}
