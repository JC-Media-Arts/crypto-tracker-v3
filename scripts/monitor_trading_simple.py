...
async def monitor_trading():
    ...
    while True:
        try:
            ...
            scans = client.client.table("scan_history").select("*").gte("timestamp", cutoff).order("scan_id", desc=True).limit(100).execute()
            ...
            recent_shadows = client.client.table("shadow_variations").select("count", count="exact").gte("created_at", recent_cutoff).execute()
            ...
            # Get recent outcomes
            if outcomes.count > 0:
                recent_outcomes = client.client.table("shadow_outcomes").select("*").order("evaluated_at", desc=True).limit(10).execute()
            ...
            hour_shadows = client.client.table("shadow_variations").select("variation_name").gte("created_at", hour_cutoff).execute()
            ...
            perf = client.client.table("shadow_performance").select("*").order("last_updated", desc=True).limit(5).execute()
