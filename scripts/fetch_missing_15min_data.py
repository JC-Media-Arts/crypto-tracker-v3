...
class Missing15MinFetcher:
    def __init__(self):
        ...
        self.missing_symbols = [
            'ALT', 'API3', 'APT', 'ARB', 'AXS', 'BAL', 'BEAM', 'BLUR', 'BONK', 'CHZ',
            'CRV', 'CTSI', 'DASH', 'ENS', 'FET', 'FIL', 'FLOKI', 'FLOW', 'GALA', 'GIGA',
            'GOAT', 'GRT', 'INJ', 'JTO', 'JUP', 'KAS', 'KSM', 'LDO', 'LRC', 'MANA',
            'MASK', 'MEME', 'MEW', 'MKR', 'MOG', 'NEAR', 'NEIRO', 'OCEAN', 'OP', 'PENDLE',
            'PEPE', 'PNUT', 'POL', 'PONKE', 'POPCAT', 'PYTH', 'QNT', 'RENDER', 'RPL', 'RUNE',
            'SAND', 'SEI', 'SHIB', 'STRK', 'STX', 'SUSHI', 'TIA', 'TON', 'TRUMP', 'TURBO',
            'WIF', 'XMR', 'ZEC'
        ]
    ...
    def check_existing_data(self, symbol: str) -> bool:
        ...
        try:
            result = self.supabase.client.table('ohlc_data').select('symbol').eq(
                'symbol', symbol
            ).eq(
                'timeframe', '15m'
            ).limit(1).execute()
