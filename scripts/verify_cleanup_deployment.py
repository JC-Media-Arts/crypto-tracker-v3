...
def check_data_to_cleanup():
    ...
    try:
        ...
        cutoffs = {
            "1-minute (>30 days)": (["1m", "1min", "1"], datetime.now() - timedelta(days=30)),
            "15-minute (>1 year)": (["15m", "15min"], datetime.now() - timedelta(days=365)),
            "1-hour (>2 years)": (["1h", "1hr", "60m", "60min"], datetime.now() - timedelta(days=730))
        }
        ...
        for description, (timeframes, cutoff) in cutoffs.items():
            try:
                ...
                for tf in timeframes:
                    result = client.client.table("ohlc_data")\
                        .select("*", count="exact")\
                        .eq("timeframe", tf)\
