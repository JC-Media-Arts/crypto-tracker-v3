...
class PaperTradingSystem:
    ...
    def __init__(self):
        ...
        self.config = {
            ...
            'symbols': [
                ...
                'DOT', 'POL', 'LINK', 'TON', 'SHIB', 'TRX', 'UNI', 'ATOM', 
                'BCH', 'APT', 'NEAR', 'ICP',
                
                # Tier 2: DeFi/Layer 2 (20 coins)
                'ARB', 'OP', 'AAVE', 'CRV', 'MKR', 'LDO', 'SUSHI', 'COMP', 
                'SNX', 'BAL', 'INJ', 'SEI', 'PENDLE', 'BLUR', 'ENS', 'GRT', 
                'RENDER', 'FET', 'RPL', 'SAND',
                
                # Tier 3: Trending/Memecoins (20 coins)
                'PEPE', 'WIF', 'BONK', 'FLOKI', 'MEME', 'POPCAT', 'MEW', 'TURBO', 
                'NEIRO', 'PNUT', 'GOAT', 'ACT', 'TRUMP', 'FARTCOIN', 'MOG', 
                'PONKE', 'TREMP', 'BRETT', 'GIGA', 'HIPPO',
                
                # Tier 4: Solid Mid-Caps (39 coins - excluding BLUR which is in Tier 2)
                'FIL', 'RUNE', 'IMX', 'FLOW', 'MANA', 'AXS', 'CHZ', 'GALA', 
                'LRC', 'OCEAN', 'QNT', 'ALGO', 'XLM', 'XMR', 'ZEC', 'DASH', 
                'HBAR', 'VET', 'THETA', 'EOS', 'KSM', 'STX', 'KAS', 'TIA', 
                'JTO', 'JUP', 'PYTH', 'DYM', 'STRK', 'ALT', 'PORTAL', 'BEAM', 
                'MASK', 'API3', 'ANKR', 'CTSI', 'YFI', 'AUDIO', 'ENJ'
            ],
            ...
            for i in range(0, len(symbols), batch_size):
                ...
                for symbol in batch:
                    try:
                        ...
                        query = self.supabase.client.table('ohlc_data').select('*').eq(
                            'symbol', symbol
    ...
    async def execute_via_hummingbot(self, signal) -> bool:
        ...
        try:
            ...
            logger.info(f"📤 Sending to Hummingbot: {signal.strategy_type.value} "
    ...
    def _display_status(self):
        ...
        logger.info(f"  Channel: ${cap.get('channel_used', 0):.0f} used, ${cap.get('channel_available', self.config['total_capital'] * self.config.get('channel_allocation', 0.3)):.0f} available")
