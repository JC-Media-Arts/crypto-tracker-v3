...
class SwingModelTrainer:
    ...
    def calculate_features(self, setups):
        ...
        for setup in setups:
            try:
                ...
                trend_score = 0
                if dist_sma20 > 0: trend_score += 0.5
                if dist_sma50 > 0: trend_score += 0.5
                if dist_sma20 > dist_sma50: trend_score += 0.5  # Accelerating trend
        ...
        return np.array(features_list), np.array(labels_list), np.array(optimal_params_list)
    ...
    def train_models(self, X, y, optimal_params):
        ...
        X_train, X_val, y_train, y_val, params_train, params_val = train_test_split(
            X_temp, y_temp, params_temp, test_size=0.25, random_state=42, stratify=y_temp
        )
