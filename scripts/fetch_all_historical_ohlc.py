...
class HistoricalOHLCFetcher:
    ...
    def get_all_symbols(self) -> List[str]:
        ...
        # Tier 1: Core coins
        tier1 = ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'ADA', 'AVAX', 'DOGE', 'DOT', 
                'POL', 'LINK', 'TON', 'SHIB', 'TRX', 'UNI', 'ATOM', 'BCH', 'APT', 'NEAR', 'ICP']
        
        # Tier 2: DeFi/Layer 2
        tier2 = ['ARB', 'OP', 'AAVE', 'CRV', 'MKR', 'LDO', 'SUSHI', 'COMP', 'SNX', 
                'BAL', 'INJ', 'SEI', 'PENDLE', 'BLUR', 'ENS', 'GRT', 'RENDER', 'FET', 'RPL', 'SAND']
        
        # Tier 3: Trending/Memecoins  
        tier3 = ['PEPE', 'WIF', 'BONK', 'FLOKI', 'MEME', 'POPCAT', 'MEW', 'TURBO', 
                'NEIRO', 'PNUT', 'GOAT', 'ACT', 'TRUMP', 'FARTCOIN', 'MOG', 'PONKE', 'TREMP', 'BRETT', 'GIGA', 'HIPPO']
        
        # Tier 4: Solid Mid-Caps
        tier4 = ['FIL', 'RUNE', 'IMX', 'FLOW', 'MANA', 'AXS', 'CHZ', 'GALA', 'LRC', 
                'OCEAN', 'QNT', 'ALGO', 'XLM', 'XMR', 'ZEC', 'DASH', 'HBAR', 'VET', 'THETA', 'EOS',
    ...
    def check_existing_data(self, symbol: str, timeframe: str) -> Tuple[Optional[datetime], int]:
        ...
        """
        try:
            result = self.supabase.client.table('ohlc_data').select(
                'timestamp'
            ).eq(
                'symbol', symbol
            ).eq(
            ...
            if result.data and len(result.data) > 0:
                ...
                # Get count
                count_result = self.supabase.client.table('ohlc_data').select(
    ...
    def fetch_ohlc_batch(self, symbol: str, timeframe: str, 
    ...
    def save_ohlc_batch(self, df: pd.DataFrame) -> int:
        ...
        for i in range(0, len(records), batch_size):
            ...
            try:
                result = self.supabase.client.table('ohlc_data').upsert(
                    batch,
    ...
    def fetch_timeframe_for_symbol(self, symbol: str, timeframe: str, 
        ...
        if force_refetch or latest_timestamp is None:
            ...
            if start_date >= end_date:
                ...
                return {'symbol': symbol, 'timeframe': timeframe, 'new_bars': 0, 'total_bars': existing_bars}
        ...
        while current_start < end_date:
            # Calculate batch end
            batch_end = min(
                current_start + timedelta(days=config['batch_days']),
                end_date
            )
    ...
    def fetch_all_timeframes_for_symbol(self, symbol: str, 
        ...
        if timeframes is None:
            ...
            timeframes = sorted(BACKFILL_CONFIG.keys(), 
                              key=lambda x: BACKFILL_CONFIG[x]['priority'])
