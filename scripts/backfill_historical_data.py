...
class HistoricalDataBackfill:
            ...
            if bars:
                ...
                if records:
                    try:
                        ...
                        total_saved += saved
                        logger.info(f"Saved {saved}/{len(records)} records for {symbol} "
    ...
    async def backfill_all(
        ...
        logger.info(f"Estimated time: {len(symbols) * chunks_per_symbol * self.rate_limit_delay / 60:.1f} minutes (paid tier - fast!)")
...
async def main():
    ...
    parser.add_argument(
        "--symbols",
        nargs="+",
        help="Specific symbols to backfill (e.g., BTC ETH SOL)"
    )
    parser.add_argument(
        "--months",
        type=int,
