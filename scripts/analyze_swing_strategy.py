...
def check_swing_trades(supabase):
    ...
    if result.data:
        ...
        for _, trade in recent.iterrows():
            print(
                f"  {trade['created_at'].strftime('%Y-%m-%d %H:%M')} - {trade['symbol']} - {trade['status']}"
            )
    else:
        ...
        if "ml_confidence" in scans_df.columns:
            ...
            if not high_conf.empty:
                ...
                for _, scan in top.iterrows():
                    print(
                        f"  {scan['symbol']} - Conf: {scan['ml_confidence']:.2f} - Decision: {scan['decision']}"
                    )
        ...
        if "features" in scans_df.columns:
            ...
            if features_list:
                ...
                if "breakout_strength" in features_df.columns:
                    print(
                        f"  Avg breakout strength: {features_df['breakout_strength'].mean():.3f}"
                    )
                    print(
                        f"  Max breakout strength: {features_df['breakout_strength'].max():.3f}"
                    )
                    strong_breakouts = features_df[
                        features_df["breakout_strength"] > 2.0
                    ]
                    print(f"  Symbols with >2% breakout: {len(strong_breakouts)}")

                if "volume_ratio" in features_df.columns:
                    print(
                        f"  Avg volume ratio: {features_df['volume_ratio'].mean():.2f}"
                    )
                ...
                if "momentum_score" in features_df.columns:
                    print(
                        f"  Avg momentum score: {features_df['momentum_score'].mean():.2f}"
                    )

                # Find best candidates
                if (
                    "breakout_strength" in features_df.columns
                    and "volume_ratio" in features_df.columns
                ):
                    features_df["combined_score"] = (
                        features_df["breakout_strength"] * features_df["volume_ratio"]
                    )
                    ...
                    for _, cand in best_candidates.iterrows():
                        print(
                            f"  {cand['symbol']}: Breakout={cand.get('breakout_strength', 0):.2f}%, "
...
def analyze_market_conditions(supabase):
    ...
    if result.data:
        ...
        for symbol in df["symbol"].unique():
            ...
            symbol_data["high_20"] = (
                symbol_data["high"].rolling(20, min_periods=5).max()
            )
            symbol_data["low_20"] = symbol_data["low"].rolling(20, min_periods=5).min()
            symbol_data["breakout_pct"] = (
                (symbol_data["close"] - symbol_data["high_20"].shift(1))
                / symbol_data["high_20"].shift(1)
                * 100
            )

            # Check for volume spikes
            symbol_data["volume_ma"] = (
                symbol_data["volume"].rolling(20, min_periods=5).mean()
            )
            symbol_data["volume_ratio"] = (
                symbol_data["volume"] / symbol_data["volume_ma"]
            )

            # Find potential breakouts
            breakouts = symbol_data[
                (symbol_data["breakout_pct"] > 2.0)
                & (  # 2% above resistance
                    symbol_data["volume_ratio"] > 1.5
                )  # 50% above average volume
            ]
        ...
        if breakout_candidates:
            ...
            # Group by symbol
            by_symbol = (
                candidates_df.groupby("symbol")
                .agg(
                    {"breakout_pct": "max", "volume_ratio": "max", "timestamp": "count"}
                )
                .round(2)
            )
            ...
            for symbol in by_symbol.head(10).index:
                ...
                print(
                    f"  {symbol}: Max breakout={row['max_breakout']:.1f}%, "
...
def check_configuration(supabase):
    ...
    query = (
        ...
        .gte("timestamp", (datetime.now(pytz.UTC) - timedelta(hours=24)).isoformat())
        .limit(1000)
    )
        ...
        mock_data = pd.DataFrame(
            {
                "timestamp": pd.date_range(
                    end=datetime.now(pytz.UTC), periods=100, freq="1H"
                ),
                "open": [100] * 100,
