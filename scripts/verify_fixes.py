...
def verify_fixes():
    ...
    processes = os.popen("ps aux | grep -E 'run_all_strategies|run_paper_trading|run_data_collector' | grep python3 | grep -v grep | wc -l").read().strip()
    ...
    process_list = os.popen("ps aux | grep -E 'run_all_strategies|run_paper_trading|run_data_collector' | grep python3 | grep -v grep").read()
    if process_list:
    ...
    print(f"   Baseline scans - DCA: {baseline_scans['DCA']}, SWING: {baseline_scans['SWING']}, CHANNEL: {baseline_scans['CHANNEL']}")
    ...
    print("|--------|--------|--------|")
    print(f"| Active Strategies | 3/3 | {sum([results.get('DCA Activity', False), results.get('SWING Activity', False), results.get('CHANNEL Activity', False)])}/3 |")
