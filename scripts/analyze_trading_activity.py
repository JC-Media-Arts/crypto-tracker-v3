        ...
        for symbol in self.symbols:
            ...
            if dca_analysis["status"] == "analyzed":
                if dca_analysis["would_trigger"]:
                    ...
                    # Track near misses
                    total_distance = (
                        dca_analysis["drop_distance"]
                        + dca_analysis["rsi_distance"] / 10
                        + dca_analysis["volume_distance"]  # Normalize RSI
                    )
            ...
            if swing_analysis["status"] == "analyzed":
                if swing_analysis["would_trigger"]:
                    ...
                    print(
                        f"     Breakout: {swing_analysis['breakout_pct']:.2f}% (need ≥ {swing_analysis['breakout_threshold']:.1f}%)"
                    )
                    print(
                        f"     Volume: {swing_analysis['volume_surge']:.2f}x (need ≥ {swing_analysis['volume_threshold']:.1f}x)"
                    )
                    print(
                        f"     Momentum: {swing_analysis['momentum']:.2f}% (need ≥ {swing_analysis['momentum_threshold']:.1f}%)"
                    )

                    # Track near misses
                    total_distance = (
                        swing_analysis["breakout_distance"]
            ...
            if channel_analysis["status"] == "analyzed":
                if channel_analysis["would_trigger"]:
                    ...
                    if (
                        channel_analysis["min_width"]
                        <= channel_analysis["channel_width"]
                        <= channel_analysis["max_width"]
                    ):
                        if channel_analysis["in_middle"]:
                            distance = min(
                                abs(0.2 - channel_analysis["position_in_channel"]),
            ...
            if near_misses[strategy]:
                ...
                for i, miss in enumerate(sorted_misses[:5], 1):
                    ...
                    if strategy == "DCA":
                        ...
                        d = miss["details"]
                        print(
                            f"     Just need: Breakout to {d['breakout_threshold']}% (currently {d['breakout_pct']:.2f}%)"
                        )
                        ...
                        if pos > 0.5:
                            print(
                                f"     Just need: Price to reach {d['channel_high']:.2f} (currently {d['current_price']:.2f})"
                            )
                        else:
                            print(
                                f"     Just need: Price to reach {d['channel_low']:.2f} (currently {d['current_price']:.2f})"
                            )
