...
<body>
    ...
    <div class="container">
        ...
        + str(trades_data["stats"]["open_count"])
        + """</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Closed Trades</div>
                <div class="stat-value">"""
        + str(trades_data["stats"]["closed_count"])
        + """</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Win Rate</div>
                <div class="stat-value" style="color: """
        + ("#22c55e" if trades_data["stats"]["win_rate"] >= 50 else "#ef4444")
        + """;">"""
        + f"{trades_data['stats']['win_rate']:.1f}%"
        + """</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Unrealized P&L %</div>
                <div class="stat-value" style="color: """
        + ("#22c55e" if trades_data["stats"]["unrealized_pnl"] >= 0 else "#ef4444")
        + """;">"""
        + f"{trades_data['stats']['unrealized_pnl']:.2f}%"
        + """</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Total P&L %</div>
                <div class="stat-value" style="color: """
        + ("#22c55e" if trades_data["stats"]["total_pnl"] >= 0 else "#ef4444")
        + """;">"""
        + f"{trades_data['stats']['total_pnl']:.2f}%"
        + """</div>
            </div>
        </div>
...
"""
    ...
    for trade in trades_data["trades"]:
        ...
        # Format limits
        sl_display = (
            f'<span class="limit-tracking">{metrics["sl_display"]}</span>'
            if metrics["sl_display"] != "Not Set"
            else '<span class="limit-not-set">Not Set</span>'
        )
        tp_display = (
            f'<span class="limit-tracking">{metrics["tp_display"]}</span>'
            if metrics["tp_display"] != "Not Set"
            else '<span class="limit-not-set">Not Set</span>'
        )
        ts_display = (
            f'<span class="limit-tracking">{metrics["ts_display"]}</span>'
            if metrics["ts_display"] != "Not Set"
            else '<span class="limit-not-set">Not Set</span>'
        )
...
def main():
    ...
    try:
        ...
        result = (
            supabase.client.table("paper_trades")
            .select("*")
        ...
        trades_data["trades"].sort(
            key=lambda x: (x["trade_status"] != "open", x["created_at"] or ""),
            reverse=False,
        )
        ...
        print(
            f"📊 Stats: {trades_data['stats']['open_count']} open, {trades_data['stats']['closed_count']} closed trades"
        )
