...
def analyze_trading_patterns(supabase) -> Dict:
    ...
    symbol_stats['win_rate'] = trades_df[trades_df['status'] == 'CLOSED'].groupby('symbol')['pnl'].apply(
        lambda x: (x > 0).mean() * 100 if len(x) > 0 else 0
    ).round(1)
    ...
    for symbol in symbol_stats.head(10).index:
        ...
        print(f"   {symbol:8} - {row['trades']:3.0f} trades, "
    ...
    print(f"   Peak trading hour: {peak_hour}:00 UTC ({hourly_entries[peak_hour]} trades)")
    ...
    if not open_trades.empty:
        ...
        open_by_symbol = open_trades.groupby('symbol').size().sort_values(ascending=False)
...
def analyze_market_conditions(supabase) -> Dict:
    ...
    for symbol in df['symbol'].unique():
        ...
        stats = {
            'symbol': symbol,
            'return_24h': (symbol_data['close'].iloc[-1] / symbol_data['close'].iloc[0] - 1) * 100,
            'volatility': returns.std(),
            'max_gain': returns.max(),
            'max_drop': returns.min(),
            'price_range': ((symbol_data['high'].max() - symbol_data['low'].min()) / symbol_data['close'].iloc[0] * 100),
    ...
    high_vol = market_df[market_df['volatility'] > market_df['volatility'].quantile(0.75)]
    low_vol = market_df[market_df['volatility'] < market_df['volatility'].quantile(0.25)]
    ...
    if not strong_up.empty:
        print(f"   Strong uptrends (>2% gain): {', '.join(strong_up['symbol'].tolist())}")
    
    strong_down = market_df[market_df['return_24h'] < -2]
    if not strong_down.empty:
        print(f"   Strong downtrends (<-2% loss): {', '.join(strong_down['symbol'].tolist())}")
...
def correlate_performance(trading_data: Dict, market_data: Dict) -> List[str]:
    ...
    if 'symbol_stats' in trading_data and 'market_df' in market_data:
        ...
        if not merged.empty:
            ...
            if 'win_rate' in merged.columns:
                ...
                if abs(corr) > 0.3:
                    if corr > 0:
                        insights.append("✅ Higher volatility correlates with better win rates")
                    else:
                        insights.append("⚠️ Higher volatility correlates with lower win rates")
            ...
            if corr > 0.3:
                insights.append("📊 System correctly identifies and trades more volatile symbols")
            
            # Best performing conditions
            if 'win_rate' in merged.columns:
                best_performers = merged[merged['win_rate'] > 60].sort_values('win_rate', ascending=False)
                if not best_performers.empty:
                    print("\n🏆 Best Performing Symbols:")
                    for _, row in best_performers.head(5).iterrows():
                        print(f"   {row['symbol']:8} - Win: {row['win_rate']:.1f}%, "
    ...
    if not trades_df.empty:
        ...
        if not channel_trades.empty:
            ...
            # Entry distribution
            if 'stop_loss' in channel_trades.columns and 'take_profit' in channel_trades.columns:
                ...
                if avg_rr < 1.5:
                    insights.append("⚠️ Risk/Reward ratio is low - consider wider take profits")
                elif avg_rr > 3:
...
def generate_recommendations(trading_data: Dict, market_data: Dict, insights: List[str]) -> List[str]:
    ...
    if 'market_df' in market_data:
        ...
        if avg_vol < 0.1:  # Low volatility
            ...
            recommendations.append("   • Consider tightening stop losses to preserve capital")
            ...
            recommendations.append("   • Consider trailing stops to capture larger moves")
    ...
    if trading_data:
        ...
        if open_trades > closed_trades * 2:
            ...
            recommendations.append("   • Review exit criteria - may be too conservative")
        
        if 'symbol_stats' in trading_data:
            symbol_stats = trading_data['symbol_stats']
            concentrated = symbol_stats[symbol_stats['trades'] > symbol_stats['trades'].mean() * 2]
            if not concentrated.empty:
                recommendations.append("\n🎯 CONCENTRATION RISK:")
                recommendations.append(f"   • Heavy concentration in {', '.join(concentrated.index.tolist())}")
    ...
    recommendations.append("\n🔧 IMMEDIATE ACTIONS:")
    recommendations.append("   1. Review and adjust CHANNEL thresholds for current volatility")
    ...
    recommendations.append("   4. Consider adding mean-reversion logic for oversold conditions")
...
def main():
    ...
    report_file = f"data/trading_behavior_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
