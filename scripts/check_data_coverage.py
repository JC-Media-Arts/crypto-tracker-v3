...
def check_data_coverage():
    ...
    try:
        ...
        symbols_result = supabase.client.table('price_data')\
            .select('symbol')\
            .execute()
        ...
        key_symbols = ['BTC', 'ETH', 'SOL', 'ADA', 'DOT', 'AVAX', 'LINK', 'UNI', 'ATOM', 'NEAR']
        ...
        for symbol in key_symbols[:20]:  # Check top 20
            ...
            if result.data:
                ...
                # Get latest
                result_latest = supabase.client.table('price_data')\
                    .select('timestamp')\
                    .eq('symbol', symbol)\
                ...
                if result_latest.data:
                    ...
                    print(f"{symbol:<10} {'~' + str(estimated_records):<11} {earliest[:10]} to {latest[:10]:<25} {days:<10}")
