...
result = db.client.table('paper_trades').select(
    'symbol, price, created_at, trading_engine, parameters'
).eq('strategy_name', 'CHANNEL').eq('side', 'BUY').gte(
...
if result.data:
    ...
    trades['minutes_ago'] = (datetime.now(timezone.utc) - trades['created_at']).dt.total_seconds() / 60
